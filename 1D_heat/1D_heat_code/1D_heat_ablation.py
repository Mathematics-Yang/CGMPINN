...
def build_save_path(problem_name: str, activation_name: str, method_name: str) -> str:
    ...
    parameter_dir = os.path.join(current_script_dir, "..", "1D_heat_parameter")
...
def plot_combined(activation_loss_histories: dict, activation_results: dict,
    ...
    x_grid = x_test.reshape(n_test, n_test).detach().cpu().numpy()
    t_grid = t_test.reshape(n_test, n_test).detach().cpu().numpy()
    ...
    line_styles = ['-', '--', '-.']
    markers = ['o', 's', '^']
    for act_idx, (activation_name, loss_histories) in enumerate(activation_loss_histories.items()):
    ...
    ax_loss.set_title('(a) Method Training Loss Comparison', pad=10)
    ...
    main_activation = list(activation_results.keys())[0]
    adam2lbfgs_res = activation_results[main_activation]['CGM']
...
def run_1D_heat_ablation():
    ...
    for activation_name, activation_fn in activations.items():
        ...
        for method_name in method_labels:
            ...
            l2_err, l2_rel_err, linf_err, _, u_pred = evaluate_model(model, test_data, u_exact)
            final_total, final_pde, final_initial, final_boundary = model.final_total_loss(
    ...
    print("\n" + "=" * 140)
    print("1D Heat Adam->L-BFGS Ablation Comparison Table")
    print("=" * 140)
