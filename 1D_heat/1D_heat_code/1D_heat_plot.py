...
# Equation parameters for 1D heat conduction
...
# Data generation function for 1D heat conduction
def generate_test_data(n_test: int) -> tuple:
    """Generate grid test data for 1D heat conduction (x∈[0,1], t∈[0,1])"""
...
# Evaluation function for 1D heat conduction
...
# 1. Basic PINN model for 1D heat conduction
class PINN(nn.Module):
    ...
    def analytical_solution(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        return (torch.sin(ALPHA1 * np.pi * x) + torch.tanh(k * x)) * torch.sin(ALPHA2 * np.pi * t)

# 2. lbPINN model (adaptive loss weights) for 1D heat conduction
...
# 3. gPINN model (gradient constraint enhancement) for 1D heat conduction
...
# 4. LNN-PINN model (Lightweight Liquid Residual Gating Block) for 1D heat conduction
...
# 5. STAR-PINN model (Stacked Adaptive Residual PINN) for 1D heat conduction
...
# 6. CGMPINN model(Curriculum-Guided GMM PINN) for 1D heat conduction
...
class CGMPINN(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, n_layers, activation, use_curriculum=True, gmm_kwargs=None, relobralo_kwargs=None):
        ...
        # ReLoBRaLo initialization (3 loss terms for heat conduction)
    ...
    def update_curriculum_weights(self, x: torch.Tensor, t: torch.Tensor) -> None:
        if not self.use_curriculum:
            return
        u_t, u_xx, _ = self.compute_gradients(x, t)
        pde_residual = u_t - u_xx - self.source_term(x, t)
...
def plot_comparison_group(loss_histories_list, solution_results_list, test_data, u_exact_np):
    ...
    # fig.suptitle("1D Heat Conduction - Solution Comparison", fontsize=16, y=0.98)
...
def generate_training_data(n_pde=1500, n_initial=300, n_boundary=300):
    """Generate training data for 1D heat conduction"""
...
def main():
    ...
    # GMM and ReLoBRaLo parameters (3 loss terms for heat conduction)
    ...
    data_dir = os.path.join(current_script_dir, "..", "1D_heat_parameter")
    ...
    model_load_configs = {
        "PINN": {
            ...
            "weight_path": os.path.join(data_dir, "1D_heat_tanh_adam2lbfgs_pinn.pth")
        },
        "lbPINN": {
            "model_class": lbPINN,
            "model_kwargs": {
            ...
            "weight_path": os.path.join(data_dir, "1D_heat_tanh_adam2lbfgs_lbpinn.pth")
        },
        "gPINN": {
            "model_class": gPINN,
            "model_kwargs": {
            ...
            "weight_path": os.path.join(data_dir, "1D_heat_tanh_adam2lbfgs_gpinn.pth")
        },
        "LNN-PINN": {
            ...
            "weight_path": os.path.join(data_dir, "1D_heat_tanh_adam2lbfgs_lnn_pinn.pth")
        },
        "STAR-PINN": {
            ...
            "weight_path": os.path.join(data_dir, "1D_heat_tanh_adam2lbfgs_star_pinn.pth")
        },
        "CGMPINN": {
            "model_class": CGMPINN,
            "model_kwargs": {
            ...
            "weight_path": os.path.join(data_dir, "1D_heat_tanh_adam2lbfgs_cgmpinn.pth")
        }
    }
    ...
    print("\n" + "="*120)
    print("Quantitative Comparison Table for 1D Heat Conduction Models (Adam→LBFGS, Tanh Activation)")
    print("="*120)
