...
# 1. define STAR-PINN model (stacked adaptive residual structure)
class STAR_PINN2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        A_z^0 = PINN0(x,y)
        A_z^1 = PINN1(x,y) + α1*A_z^0
        A_z^2 = PINN2(x,y) + α2*A_z^1
        ...
        final output = fusion result of the last block
        """
        # output of the first block
        current_output = self.pinn_blocks[0](x, y)
        
        # subsequent blocks: residual fusion
        for i in range(1, self.n_blocks):
            block_output = self.pinn_blocks[i](x, y)
            # adaptive weight fusion (weights constrained in [0,1])
...
def train_2D_poisson_STAR_PINN_with_activations_and_source_term():
    ...
    # calculate exact solution for test data
    model_temp = STAR_PINN2D(input_dim, output_dim, hidden_dim, n_layers_per_block, TanhActivation())
    ...
    for activation_name, activation_fn in activations.items():
        ...
        model_adam = STAR_PINN2D(input_dim, output_dim, hidden_dim, n_layers_per_block, activation_fn)
        ...
        model_lbfgs = STAR_PINN2D(input_dim, output_dim, hidden_dim, n_layers_per_block, activation_fn)
        loss_history_lbfgs = []
        start_time = time.time()
        train_with_lbfgs(
            model=model_lbfgs,
        ...
        model_adam_lbfgs = STAR_PINN2D(input_dim, output_dim, hidden_dim, n_layers_per_block, activation_fn)
        ...
        if activation_name == 'Tanh':  
            save_path = "2D_poisson_tanh_adam2lbfgs_star_pinn.pth"
            complete_save_dict = {
    ...
    print("\n" + "="*120)
    print("Comprehensive comparison table for all activation functions × optimizers (2D Poisson Equation - STAR-PINN)")
    print("="*120)
    ...
    print("\n" + "="*100)
    print("Best performance per activation function (sorted by standard L2 error - STAR-PINN)")
    print("="*100)
    for activation_name in activation_names:
        print(f"\n【{activation_name}】")
        optim_results = []
...
train_2D_poisson_STAR_PINN_with_activations_and_source_term()