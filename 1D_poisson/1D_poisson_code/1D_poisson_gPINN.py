...
class gPINN(nn.Module):
    ...
    # compute high-order gradients for gPINN
    def compute_high_order_gradients(self, x: torch.Tensor) -> tuple:
        ...
        # PDE residual res = u_xx - f(x)
        f_exact = self.source_term(x)
        res = u_xx - f_exact
        
        # first-order gradient of residual res_x
    ...
    def final_total_loss(self, pde_data) -> tuple[float, float, float, float]:
        x_pde = pde_data
        
        with torch.enable_grad():
            pde_loss_val = self.pde_loss(x_pde).item()
            boundary_loss_val = self.boundary_loss().item()
            grad_loss_val = self.grad_loss(x_pde).item()
...
def train_with_optimizer_gpinn(
    ...
    for epoch in range(epochs):
        ...
        grad_loss = model.grad_loss(pde_data)
...
def plot_solutions_by_activation_and_optimizer(
    ...
    for activation_name, results in activation_results.items():
        ...
        for idx, optim_label in enumerate(optim_labels):
            ...
            ax1.plot(x_test_np, u_pred, 'r--', label='gPINN Prediction', linewidth=2, alpha=0.8)
...
# 8. main training and evaluation routine
def train_1D_poisson_gPINN_with_activations_and_source_term():
    ...
    model_temp = gPINN(input_dim, output_dim, hidden_dim, 1, TanhActivation())
    ...
    # training by activation function
    for activation_name, activation_fn in activations.items():
        print("\n" + "="*80)
        print(f"Starting training - Activation Function: {activation_name} (gPINN)")
        print("="*80)
        ...
        final_total, final_pde, final_boundary, final_grad = model_adam.final_total_loss(pde_data)
        ...
        final_total, final_pde, final_boundary, final_grad = model_lbfgs.final_total_loss(pde_data)
        ...
        final_total, final_pde, final_boundary, final_grad = model_adam_lbfgs.final_total_loss(pde_data)
        ...
        if activation_name == 'Tanh':  
            # define save path
            save_path = "1D_poisson_tanh_adam2lbfgs_gpinn.pth"
    
            # pack all contents to save
            complete_save_dict = {
                'model_state_dict': model_adam_lbfgs.state_dict(),  # model learnable parameters 
    ...
    print("\n" + "="*140)
    print("gPINN 1D Poisson Equation Comprehensive Comparison")
    print("="*140)
    ...
    print("\n" + "="*120)
    print("gPINN Best Performance of Each Activation Function (Sorted by L2 Error)")
    print("="*120)
    for activation_name in activation_names:
        print(f"\n【{activation_name}】")
        optim_results = []
        ...
        for i, res in enumerate(optim_results_sorted, 1):
            print(
                ...
                f"最终损失: {res['final_loss']:.2e} | 最终梯度损失: {res['final_grad_loss']:.2e}"
            )

# execute the training and evaluation routine
train_1D_poisson_gPINN_with_activations_and_source_term()