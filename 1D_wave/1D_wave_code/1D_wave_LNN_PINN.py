...
# LNN-PINN Liquid Residual Gated Block (Core Module)
class LiquidResidualBlock(nn.Module):
    """
    Lightweight Liquid Residual Gated Block
    Core: Learnable gating parameters α, β, adaptively regulate information flow, maintain model compactness
    ...
    def forward(self, x):
        ...
        # 3. Liquid gated fusion (adaptively regulate the proportion of new/old information)
        alpha = self.softplus(self.alpha)  # Ensure non-negativity
        beta = self.softplus(self.beta)    # Ensure non-negativity
...
def train_adam_lbfgs(
    ...
    """Two-stage training: first global exploration with Adam, then local refinement with L-BFGS"""
    print("\n=== Stage 1: Global Exploration with Adam ===")
...
# 8. Main function
def train_1D_wave_LNN_PINN_with_activations():
    ...
    for activation_name, activation_fn in activations.items():
        ...
        # Save Tanh+Adam→L-BFGS model
        if activation_name == 'Tanh':  
            save_path = "1D_wave_tanh_adam2lbfgs_lnn_pinn.pth"
            complete_save_dict = {
            ...
            print(f"✅ Tanh+Adam→L-BFGS wave equation LNN-PINN model saved to: {save_path}")
    ...
    print("\n" + "="*120)
    print("All Activation Functions × Optimizers Comprehensive Comparison Table (1D Wave Equation with Damping - LNN-PINN)")
    print("="*120)
...
train_1D_wave_LNN_PINN_with_activations()