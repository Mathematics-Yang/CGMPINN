...
class gPINN(nn.Module):
    ...
    def compute_high_order_gradients(self, x: torch.Tensor, t: torch.Tensor) -> tuple:
        ...
        f_x = torch.autograd.grad(
            ...
            inputs=x,
            grad_outputs=torch.ones_like(pde_residual),
            create_graph=True,
            retain_graph=True,
            only_inputs=True
        )[0]
        ...
        return u_t, u_tt, u_xx, pde_residual, f_x, f_t

    def pde_loss(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        _, u_tt, _, u_xx = self.compute_gradients(x, t)
        pde_residual = u_tt + 2 * BETA * self.compute_gradients(x, t)[0] - (c ** 2) * u_xx
        return torch.mean(pde_residual ** 2)
    
    def grad_loss(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        _, _, _, _, f_x, f_t = self.compute_high_order_gradients(x, t)
...
def train_with_lbfgs_gpinn(
    ...
    def closure() -> torch.Tensor:
        ...
        optimizer.zero_grad()
        # Calculate losses (including gradient loss)
        pde_loss = model.pde_loss(x_pde, t_pde)
        initial_loss = model.initial_loss(x_initial, t_initial)
        boundary_loss = model.boundary_loss(t_boundary)
        grad_loss = model.grad_loss(x_pde, t_pde)
...
def train_adam_lbfgs_gpinn(
    ...
    """Two-stage training: first Adam global exploration, then L-BFGS local fine-tuning"""
...
# 8. Main function
def train_1D_wave_gPINN_with_activations():
    ...
    for activation_name, activation_fn in activations.items():
        ...
        # 4.3 Adam→L-BFGS training (gPINN)
        ...
        # Save Tanh+Adam→L-BFGS model (gPINN)
        if activation_name == 'Tanh':  
            save_path = "1D_wave_tanh_adam2lbfgs_gpinn.pth"
            complete_save_dict = {
            ...
            print(f"✅ Tanh+Adam→LBFGS wave equation gPINN model saved to: {save_path}")
    ...
    # 6. Print comprehensive comparison table (with gradient loss column)
    print("\n" + "="*140)
    print("Comprehensive comparison table for all activation functions × optimizers (1D wave equation with damping term gPINN)")
    print("="*140)
    ...
    print("\n" + "="*120)
    print("Best performance for each activation function (sorted by standard L2 error, gPINN)")
    print("="*120)
    for activation_name in activation_names:
        print(f"\n【{activation_name}】")
        optim_results = []
...
train_1D_wave_gPINN_with_activations()