...
class STAR_PINN(nn.Module):
    ...
    # forward pass: stacked adaptive residual fusion + adapt to x,t dual inputs
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """
        forward pass: stacked adaptive residual fusion + input x,t concatenation
        A_z^0 = PINN0([x,t])
...
# 8. Main function
def train_1D_wave_STAR_PINN_with_activations():
    ...
    for activation_name, activation_fn in activations.items():
        ...
        if activation_name == 'Tanh':  
            save_path = "1D_wave_tanh_adam2lbfgs_star_pinn.pth"
            complete_save_dict = {
            ...
            print(f"✅ Tanh+Adam→LBFGS wave equation STAR-PINN model saved to: {save_path}")
    ...
    print("\n" + "="*120)
    print("Comprehensive comparison table for all activation functions × optimizers (1D wave equation with damping - STAR-PINN)")
    print("="*120)
...
train_1D_wave_STAR_PINN_with_activations()