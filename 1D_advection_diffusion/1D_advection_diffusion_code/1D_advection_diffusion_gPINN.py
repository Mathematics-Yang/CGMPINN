...
class gPINN(nn.Module):
    ...
    def compute_high_order_gradients(self, x: torch.Tensor, t: torch.Tensor) -> tuple:
        ...
        return u_t, u_x, u_xx, f, f_x, f_t

    def pde_loss(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        _, _, _, f, _, _ = self.compute_high_order_gradients(x, t)
...
def generate_data(
    ...
    # PDE interior points: x ∈ [-1,1], t ∈ [0,1]
...
# 3. training function (Adam, with gradient loss)
...
def plot_solutions_by_activation_and_optimizer(
    ...
    for activation_name, results in activation_results.items():
        ...
        fig.suptitle(f'Solution Comparison - Activation: {activation_name} (gPINN)', fontsize=24, y=0.99)
...
def train_1D_advection_diffusion_gPINN():
    # 1. Hyperparameter settings
    n_layers = 4              
    input_dim = 2             
    output_dim = 1          
    hidden_dim = 50  
    grad_weight = 0.01  # Gradient loss weight      
    n_pde = 3000             
    n_initial = 300          
    n_boundary = 300         
    n_test = 100              
    ...
    # Calculate analytical solution for advection-diffusion equation
    pi = np.pi
    ...
    for activation_name, activation_fn in activations.items():
        ...
        if activation_name == 'Tanh':
            save_path = "1D_advection_diffusion_tanh_adam2lbfgs_gpinn.pth"
            complete_save_dict = {
            ...
            print(f"✅ Tanh+Adam→L-BFGS gPINN model saved to: {save_path}")
    ...
    # 6. Print comprehensive comparison table (including gradient loss)
    print("\n" + "="*140)
    print("1D Advection-Diffusion Equation - gPINN Comprehensive Comparison Table (Including Gradient Loss)")
    print("="*140)
    ...
    print("\n" + "="*120)
    print("Best performance for each optimizer (sorted by standard L2 error)")
    print("="*120)
    for activation_name in activation_names:
        print(f"\n【{activation_name}】")
        optim_results = []
...
train_1D_advection_diffusion_gPINN()