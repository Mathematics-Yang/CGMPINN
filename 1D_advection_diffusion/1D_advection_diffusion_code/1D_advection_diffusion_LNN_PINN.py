...
def train_1D_advection_diffusion_LNN_PINN():
    ...
    # Calculate analytical solution of the advection-diffusion equation
    pi = np.pi
    ...
    for activation_name, activation_fn in activations.items():
        print("\n" + "="*80)
        print(f"Starting training - Activation function: {activation_name} | Advection-diffusion equation parameters: β={BETA}, ν={NU}")
        print("="*80)
        ...
        if activation_name == 'Tanh':
            save_path = "1D_advection_diffusion_tanh_adam2lbfgs_lnn_pinn.pth"
            complete_save_dict = {
    ...
    print("\n" + "="*120)
    print("Advection-Diffusion Equation - LNN-PINN Comprehensive Comparison Table (Liquid Residual Gating Version)")
    print("="*120)
    ...
    print("\n" + "="*100)
    print("Best performance of each optimizer (sorted by standard L2 error - LNN-PINN)")
    print("="*100)
    for activation_name in activation_names:
        print(f"\n【{activation_name}】")
        optim_results = []
...
train_1D_advection_diffusion_LNN_PINN()