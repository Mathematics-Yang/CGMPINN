...
class STAR_PINN(nn.Module):
    ...
    # Forward pass: stacked residual fusion + adapt to x,t dual inputs
...
# 5. Two-stage training function (Adam followed by L-BFGS)
...
# 6. Evaluation function (including various error metrics)
def evaluate_model(
    model: STAR_PINN,
    test_data: tuple,
...
def train_1D_advection_diffusion_STAR_PINN():
    # 1. Hyperparameter settings
    n_layers_per_block = 2    # Number of hidden layers per STAR-PINN block
    input_dim = 2             
    output_dim = 1          
    hidden_dim = 50        
    n_pde = 3000             
    ...
    # Calculate the analytical solution of the advection-diffusion equation
    pi = np.pi
    ...
    for activation_name, activation_fn in activations.items():
        print("\n" + "="*80)
        print(f"Starting training - Activation function: {activation_name} | Advection-diffusion parameters: β={BETA}, ν={NU}")
        ...
        if activation_name == 'Tanh':
            save_path = "1D_advection_diffusion_tanh_adam2lbfgs_star_pinn.pth"
            complete_save_dict = {
    ...
    print("\n" + "="*120)
    print("Advection-Diffusion Equation - STAR-PINN Comprehensive Comparison Table")
    print("="*120)
...
train_1D_advection_diffusion_STAR_PINN()