...
# Fisher-KPP equation parameters and domain 
...
def generate_test_data(n_test: int) -> tuple:
    """Generate grid test data for Fisher-KPP (x∈[-5,5], t∈[0,2])"""
...
class gPINN(nn.Module):
    ...
    def compute_high_order_gradients(self, x: torch.Tensor, t: torch.Tensor) -> tuple:
        ...
        # Fisher-KPP PDE residual and its gradients
        res = u_t - D * u_xx - r * u * (1 - u)
        ...
        return u_t, u_xx, res, res_x, res_t
    ...
    def pde_loss(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        _, _, res, _, _ = self.compute_high_order_gradients(x, t)
...
class CGMPINN(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, n_layers, activation, use_curriculum=True, gmm_kwargs=None, relobralo_kwargs=None):
        ...
        # ReLoBRaLo initialization (3 loss terms for Fisher-KPP)
    ...
    # Fisher-KPP curriculum weight update
    ...
    def boundary_loss(self, t: torch.Tensor) -> torch.Tensor:
        x0 = torch.full_like(t, x_min, requires_grad=True).to(next(self.parameters()).device)
        u0_pred = self.forward(x0, t)
        u0_exact = self.analytical_solution(x0, t)
        
        x1 = torch.full_like(t, x_max, requires_grad=True).to(next(self.parameters()).device)
        u1_pred = self.forward(x1, t)
        u1_exact = self.analytical_solution(x1, t)
...
def plot_comparison_group(loss_histories_list, solution_results_list, test_data, u_exact_np):
    """Comparison plot for Fisher-KPP reaction-diffusion equation"""
    ...
    # 2. Solution and error comparison 
    ...
    for idx in range(n_models):
        ...
        ax1 = axes[idx, 0]
        im1 = ax1.pcolormesh(t_test_grid, x_test_grid, u_pred_grid, cmap=cm.jet, shading='gouraud', vmin=0, vmax=1)
        ...
        # Exact solution
        ax2 = axes[idx, 1]
        im2 = ax2.pcolormesh(t_test_grid, x_test_grid, u_exact_grid, cmap=cm.jet, shading='gouraud', vmin=0, vmax=1)
        ax2.set_xlabel("Time (t)", fontsize=10)
        ax2.set_ylabel("Position (x)", fontsize=10)
        ax2.set_title("Analytical Solution (Traveling Wave)", fontsize=12)
...
def plot_error_analysis_group(solution_results_list, test_data, model_names=None):
    ...
    x_vals = np.linspace(x_min, x_max, n_test)
    t_vals = np.linspace(t_min, t_max, n_test)
...
def plot_comparison_group_error(loss_histories_list, solution_results_list, test_data, u_exact_np):
    model_names = ["PINN", "lbPINN", "gPINN", "LNN-PINN", "STAR-PINN", "CGMPINN"]
    x_test, t_test = test_data
    n_test = int(np.sqrt(len(x_test)))
    cgmpinn_result = solution_results_list[5]
    fig, axes = plt.subplots(1, 4, figsize=(24, 5))
    # fig.suptitle('1D Heat Conduction - PINN Comparison & CGMPINN Error Analysis', fontsize=16, y=0.98)
...
def plot_wavefront_tracking(models_dict, threshold=0.5, n_times=50, n_x=500):
    x_vals = torch.linspace(x_min, x_max, n_x).reshape(-1, 1)
    t_vals_list = np.linspace(t_min + 0.01, t_max, n_times)
    ...
    for t_val in t_vals_list:
        ...
        try:
            idx = np.where(np.diff(np.sign(u_exact - threshold)))[0]
            if len(idx) > 0:
                i = idx[0]
                x_front = x_np[i] + (threshold - u_exact[i]) * (x_np[i+1] - x_np[i]) / (u_exact[i+1] - u_exact[i])
                wavefront_exact.append((t_val, x_front))
        except:
            pass
    
    ax1 = axes[0]
    if wavefront_exact:
        t_ex, x_ex = zip(*wavefront_exact)
        ax1.plot(t_ex, x_ex, 'k-', linewidth=2.5, label='Exact', marker='o', markersize=2)
    
    t_theory = np.linspace(t_min, t_max, 100)
    if wavefront_exact:
        x0_exact = wavefront_exact[0][1] - c * wavefront_exact[0][0]
        ax1.plot(t_theory, x0_exact + c * t_theory, 'b:', linewidth=2, label=f'Theory')
    ...
    for idx, (name, model) in enumerate(models_dict.items()):
        ...
        for t_val in t_vals_list:
            ...
            try:
                idx_cross = np.where(np.diff(np.sign(u_pred - threshold)))[0]
                if len(idx_cross) > 0:
                    i = idx_cross[0]
                    x_front = x_vals.numpy().flatten()[i]
                    wavefront_pred.append((t_val, x_front))
            except:
                pass
        
        if wavefront_pred:
            t_pr, x_pr = zip(*wavefront_pred)
            ax1.plot(t_pr, x_pr, '--', color=colors[idx+1], linewidth=1.5, label=name)
    
    ax1.set_xlabel('Time $t$', fontsize=12)
    ax1.set_ylabel('Wavefront Position $x_f$', fontsize=12)
    ax1.set_title('Wavefront Tracking', fontsize=13)
    ax1.legend(fontsize=9)
    
    ax2 = axes[1]
    if len(wavefront_exact) > 1:
        t_ex, x_ex = zip(*wavefront_exact)
        speed_exact = np.gradient(x_ex, t_ex)
        ax2.plot(t_ex, speed_exact, 'k-', linewidth=2, label='Exact')
    
    ax2.axhline(y=c, color='b', linestyle=':', linewidth=2, label=f'Theory')
    ...
    for idx, (name, model) in enumerate(models_dict.items()):
        ...
        if len(wavefront_pred) > 1:
            t_pr, x_pr = zip(*wavefront_pred)
            speed_pred = gaussian_filter1d(np.gradient(x_pr, t_pr), sigma=2)
            ax2.plot(t_pr, speed_pred, '--', color=colors[idx+1], linewidth=1.5, label=name)
    
    ax2.set_xlabel('Time $t$', fontsize=12)
    ax2.set_ylabel('Wave Speed $c$', fontsize=12)
    ax2.set_title('Wave Speed Estimation', fontsize=13)
    ax2.legend(fontsize=9)
    ax2.set_ylim(c*0.5, c*1.5)
    ...
    for idx, (name, model) in enumerate(models_dict.items()):
        ...
        if len(wavefront_pred) > 1:
            ...
            speed_error = np.abs(np.nan_to_num(speed_pred) - c) / c * 100
    ...
    ax3.set_xlabel('Time $t$', fontsize=12)
    ax3.set_ylabel('Wave Speed Error (%)', fontsize=12)
    ax3.set_title('Wave Speed Relative Error', fontsize=13)
    ax3.legend(fontsize=9)
    
    plt.tight_layout()
    plt.show()

def plot_combined_analysis_plus(loss_histories_list, solution_results_list, test_data, u_exact_np, models_dict, threshold=0.5, n_times=50, n_x=500):
    model_names = ["PINN", "lbPINN", "gPINN", "LNN-PINN", "STAR-PINN", "CGMPINN"]
    
    fig, axes = plt.subplots(1, 4, figsize=(32, 6))  
    # fig.suptitle('Fisher-KPP Reaction-Diffusion Model Analysis', fontsize=16, y=1.02)  
    ...
    for name, model in models_dict.items():
        ...
        model_wavefront_data[name] = wavefront_pred
    ...
    if wavefront_exact:
        x0_exact = wavefront_exact[0][1] - c * wavefront_exact[0][0]
        ax2.plot(t_theory, x0_exact + c * t_theory, 'b:', linewidth=2, label='Theory')
    
    for idx, (name, wavefront_pred) in enumerate(model_wavefront_data.items()):
        if wavefront_pred:
            t_pr, x_pr = zip(*wavefront_pred)
            ax2.plot(t_pr, x_pr, '--', color=colors[idx+1], linewidth=1.5, label=name)
    
    ax2.set_xlabel('Time $t$', fontsize=12)
    ax2.set_ylabel('Wavefront Position $x_f$', fontsize=12)
    ax2.set_title('(b) Wavefront Tracking', fontsize=13)
    ...
    if len(wavefront_exact) > 1:
        t_ex, x_ex = zip(*wavefront_exact)
        speed_exact = np.gradient(x_ex, t_ex)
        ax3.plot(t_ex, speed_exact, 'k-', linewidth=2, label='Exact')
    
    ax3.axhline(y=c, color='b', linestyle=':', linewidth=2, label='Theory')
    
    for idx, (name, wavefront_pred) in enumerate(model_wavefront_data.items()):
        if len(wavefront_pred) > 1:
            t_pr, x_pr = zip(*wavefront_pred)
            speed_pred = gaussian_filter1d(np.gradient(x_pr, t_pr), sigma=2)
            ax3.plot(t_pr, speed_pred, '--', color=colors[idx+1], linewidth=1.5, label=name)
    
    ax3.set_xlabel('Time $t$', fontsize=12)
    ax3.set_ylabel('Wave Speed $c$', fontsize=12)
    ax3.set_title('(c) Wave Speed Estimation', fontsize=13)
    ax3.legend(fontsize=8)
    ax3.set_ylim(c*0.5, c*1.5)
    ...
    for idx, (name, wavefront_pred) in enumerate(model_wavefront_data.items()):
        if len(wavefront_pred) > 1:
            ...
            speed_error = np.abs(np.nan_to_num(speed_pred) - c) / c * 100
            speed_error_smooth = gaussian_filter1d(speed_error, sigma=2)
            ax4.plot(t_pr, speed_error_smooth, color=colors[idx+1], linewidth=1.5, label=name)
    
    ax4.set_xlabel('Time $t$', fontsize=12)
    ax4.set_ylabel('Wave Speed Error (%)', fontsize=12)
    ax4.set_title('(d) Wave Speed Relative Error', fontsize=13)
...
def plot_reaction_diffusion_balance(model, model_name="Model", time_points=[0.5, 1.0, 1.5], n_x=300):
    x_vals = torch.linspace(x_min, x_max, n_x).reshape(-1, 1).requires_grad_(True)
    ...
    for i, t_val in enumerate(time_points):
        ...
        diffusion_term = (D * u_xx).detach().numpy().flatten()
        reaction_term = (r * u * (1 - u)).detach().numpy().flatten()
        residual = u_t_np - diffusion_term - reaction_term
        
        ax1 = axes[0, i] if n_times > 1 else axes[0]
        ax1.plot(x_np, u_t_np, 'k-', linewidth=2, label=r'$\partial u/\partial t$')
        ax1.plot(x_np, diffusion_term, 'b--', linewidth=2, label=r'$D \nabla^2 u$')
        ax1.plot(x_np, reaction_term, 'r-.', linewidth=2, label=r'$ru(1-u)$')
        ax1.axhline(y=0, color='gray', linestyle=':', alpha=0.5)
        ax1.fill_between(x_np, 0, diffusion_term, alpha=0.2, color='blue')
        ax1.fill_between(x_np, 0, reaction_term, alpha=0.2, color='red')
        ax1.set_xlabel('$x$', fontsize=12)
        ax1.set_ylabel('Term Value', fontsize=12)
        ax1.set_title(f'{subplot_labels[label_idx]} $t = {t_val}$: Term Contributions', fontsize=12)
        label_idx += 1
        ...
        ax2_twin.set_ylabel('Residual', fontsize=12, color='orange')
        ...
        label_idx += 1
        ax2.legend([line1, line2], ['Solution $u$', 'PDE Residual'], fontsize=9, loc='upper right')
    
    # plt.suptitle(f'{model_name}: Reaction-Diffusion Balance Analysis', fontsize=14, y=1.02)
    plt.tight_layout()
    plt.show()

# generate training data for Fisher-KPP
def generate_training_data(n_pde=1500, n_initial=300, n_boundary=300):
    """Generate training data for Fisher-KPP (x∈[-5,5], t∈[0,2])"""
    # PDE points (x, t) ∈ [-5,5]×[0,2]
    ...
    # Boundary condition points (x=x_min/x_max, t∈[0,2])
...
def main():
    ...
    # GMM and ReLoBRaLo parameters (3 loss terms for Fisher-KPP)
    ...
    # 2. Compute analytical solution (Fisher-KPP) for comparison
    ...
    data_dir = os.path.join(current_script_dir, "..", "1D_fisher_kpp_parameter")
    ...
    model_load_configs = {
        "PINN": {
            ...
            "weight_path": os.path.join(data_dir, "1D_fisher_kpp_tanh_adam2lbfgs_pinn.pth")
        },
        "lbPINN": {
            "model_class": lbPINN,
            "model_kwargs": {
            ...
            "weight_path": os.path.join(data_dir, "1D_fisher_kpp_tanh_adam2lbfgs_lbpinn.pth")
        },
        "gPINN": {
            "model_class": gPINN,
            "model_kwargs": {
            ...
            "weight_path": os.path.join(data_dir, "1D_fisher_kpp_tanh_adam2lbfgs_gpinn.pth")
        },
        "LNN-PINN": {
            ...
            "weight_path": os.path.join(data_dir, "1D_fisher_kpp_tanh_adam2lbfgs_lnn_pinn.pth")
        },
        "STAR-PINN": {
            ...
            "weight_path": os.path.join(data_dir, "1D_fisher_kpp_tanh_adam2lbfgs_star_pinn.pth")
        },
        "CGMPINN": {
            "model_class": CGMPINN,
            "model_kwargs": {
            ...
            "weight_path": os.path.join(data_dir, "1D_fisher_kpp_tanh_adam2lbfgs_cgmpinn.pth")
        }
    }
    ...
    plot_comparison_group_error(group_losses, group_solutions, test_data, u_exact_np)
    plot_wavefront_tracking({
        "PINN": load_pinn_model(
            model_class=PINN,
            model_kwargs=model_load_configs["PINN"]["model_kwargs"],
            weight_path=model_load_configs["PINN"]["weight_path"],
            device=device
        )[0],
        "lbPINN": load_pinn_model(
            model_class=lbPINN,
            model_kwargs=model_load_configs["lbPINN"]["model_kwargs"],
            weight_path=model_load_configs["lbPINN"]["weight_path"],
            device=device
        )[0],
        "gPINN": load_pinn_model(
            model_class=gPINN,
            model_kwargs=model_load_configs["gPINN"]["model_kwargs"],
            weight_path=model_load_configs["gPINN"]["weight_path"],
            device=device
        )[0],
        "LNN-PINN": load_pinn_model(
            model_class=LNN_PINN,
            model_kwargs=model_load_configs["LNN-PINN"]["model_kwargs"],
            weight_path=model_load_configs["LNN-PINN"]["weight_path"],
            device=device
        )[0],
        "STAR-PINN": load_pinn_model(
            model_class=STAR_PINN,
            model_kwargs=model_load_configs["STAR-PINN"]["model_kwargs"],
            weight_path=model_load_configs["STAR-PINN"]["weight_path"],
            device=device
        )[0],
        "CGMPINN": load_pinn_model(
            model_class=CGMPINN,
    ...
    plot_reaction_diffusion_balance(
        load_pinn_model(
            model_class=CGMPINN,
            model_kwargs=model_load_configs["CGMPINN"]["model_kwargs"],
            weight_path=model_load_configs["CGMPINN"]["weight_path"],
    ...
    print("\n" + "="*120)
    print("Quantitative Comparison Table for 1D Fisher-KPP Models (Adam→LBFGS, Tanh Activation)")
    print("="*120)
