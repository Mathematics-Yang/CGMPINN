...
def build_save_path(problem_name: str, activation_name: str, method_name: str) -> str:
    ...
    parameter_dir = os.path.join(current_script_dir, "..", "1D_fisher_kpp_parameter")
...
def plot_combined_wave(activation_loss_histories: dict, activation_results: dict,
    ...
    im_pred = ax_pred.pcolormesh(t_grid, x_grid, u_pred_grid, cmap=cm.jet, shading='gouraud', vmin=0, vmax=1)
    ...
    im_exact = ax_exact.pcolormesh(t_grid, x_grid, u_exact_grid, cmap=cm.jet, shading='gouraud', vmin=0, vmax=1)
...
if __name__ == "__main__":
    def run_1D_fisher_kpp_ablation():
        n_layers = 4
        input_dim = 2
        output_dim = 1
        hidden_dim = 80
        ...
        plot_combined_wave(activation_loss_histories, activation_results, test_data, u_exact_np, method_labels)

        print("\n" + "=" * 140)
        print("1D Fisher-KPP Adam->L-BFGS Ablation Comparison Table")
        print("=" * 140)
